 ...
 * @param body the body to attach to the image
 * @return a pointer to the newly allocated image asset
 */
...
/**
 * Allocates memory for an image asset with an attached body, rotated at a specific angle. 
 * When the asset is rendered, the image will be rendered on top of the body.
 * 
 * @param filepath the path to the image file
 * @param bounding_box the bounding box containing the location and dimensions
 * of the asset when it is rendered
 * @param body the body to attach to the image
 * @param angle the angle at which to rotate the body
 * @return a pointer to the newly allocated image asset
*/
asset_t *asset_make_image_with_body_angle(const char *filepath, SDL_Rect bounding_box,
                                    body_t *body, double angle);

/**
 * Retrieves the corresponding body of an asset IF the asset is an image. 
 * 
 * @param asset the asset to retrieve the body from
*/
